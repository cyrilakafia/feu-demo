...
def run_inference(filename, n_gibbs_iters):

    SEED = 1234
    np.random.seed(SEED)

    network_df = load_network(filename)
    ...
    output = infer_dp(obs_all, calc_bssm_log_like_cue, 1.0, samp_base, base_logpdf, n_gibbs_iters=n_gibbs_iters, 
                    sds_mh_proposal=1/2 * np.array([0.5, 0.5]), n_aux=5, dump_file='inference/pickle.p', seed=SEED)
        
    if os.path.exists('inference/pickle.p') and not os.path.exists('inference/pickle_2.p'):
        output = infer_dp(obs_all, calc_bssm_log_like_cue, 1.0, samp_base, base_logpdf, n_gibbs_iters=n_gibbs_iters, 
                    sds_mh_proposal=1/2 * np.array([0.5, 0.5]), n_aux=5, dump_file='inference/pickle_2.p', seed=SEED)
